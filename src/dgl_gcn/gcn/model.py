...
class SAGE(tf.keras.Model):
    ...
    def __init__(self,
        ...
        for _ in range(num_mp_stages):
            ...
            if agg_type == 'MeanPool':
                self.aggregators.append(MeanPool(agg_dim,activation=agg_activation))
            if agg_type == 'MeanPoolEdgeData':
                self.aggregators.append(MeanPoolEdgeData(agg_dim,activation=agg_activation))
    ...
    def call(self,graph,mode='sampling'):
        '''NOTE: doesn't need to have the 'features' reset like for 'no_sampling' case,
        since, each batch is copied (with initial value of 'h' == 'features') from the
        parent graph each batch.
        '''
        if mode == 'sampling':
