...
class PackingEnv(gym.Env):
    ...
    def __init__(self, env_config):
        ...
        self.observation_space = spaces.Box(low=-10000,high=10000,shape=(OBS_SPACE_DIM,))
...
#             reward = self._get_reward(allocated=False)
    ...
    def _get_observation(self):
        ...
        packing_eff = self.manager.packing_efficiency('Resource')
        cur_req_hold_time = self.manager.allocated_requests[self.current_request].requirements['holding_time']

        return np.array([packing_eff,cur_req_hold_time])
...
#         print('UPDATING REQUEST')
    ...
    def _check_holding_times(self):
        # print('checking holding times')
        for req in self.manager.allocated_requests.keys():
            # print(req)
            # print('allocated: {}'.format(self.manager.allocated_requests[req].requirements['allocated']))
            # print('alloc time: {}'.format(self.manager.allocated_requests[req].requirements['allocation_time']))
            ...
            if self.manager.allocated_requests[req].requirements['allocated'] == True:
                if (self.time - self.manager.allocated_requests[req].requirements['allocation_time']) == self.manager.allocated_requests[req].requirements['holding_time']:             
                    # print('components: {}'.format(list(self.manager.allocated_requests[req].allocations.keys())))
                    self.manager.de_allocate(req)
                    self.manager.allocated_requests[req].requirements['allocated'] = False
                    # print('deallocating {}'.format(req))
