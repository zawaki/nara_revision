...
class NonParametricActionsModel(TFModelV2):
    ...
    def __init__(self,
                 ...
                 action_embed_size=EMBEDDING_DIM,
                 **kw):
        super(NonParametricActionsModel, self).__init__(
            obs_space, action_space, num_outputs, model_config, name, **kw)
        '''NOTE:
        Currenty using a FullyConnectedNetwork since that's what is used in the
        rllib parametric_actions_model example. Doesn't seem to be necessary, 
        ...
        if self.use_gnn:
            logits_input_dim = model_config['custom_model_config']['obs_emb_dim'] + \
                                action_space.n*model_config['custom_model_config']['agg_dim']
        else:
            logits_input_dim = model_config['custom_model_config']['obs_emb_dim'] + \
                               len(self.config['custom_model_config']['features']['node'])

        self.action_logits_model = tf.keras.Sequential()
        self.action_logits_model.add(tf.keras.layers.Dense(action_space.n,input_shape=(logits_input_dim+model_config['custom_model_config']['agg_dim'],),name='logits_dense'))
        self.register_variables(self.action_logits_model.variables)
...
#         print('step embeddings: {}'.format(embedded_actions))
        ...
        # print('obs shape: {}'.format(action_embed.shape))
        # print('embed shape: {}'.format(embedded_actions.shape))
        embs = tf.reshape(embedded_actions,[embedded_actions.shape[0],embedded_actions.shape[1]*embedded_actions.shape[2]])
        # print('obs vec shape: {}'.format(action_embed.shape))
        # print('embs vec shape: {}'.format(embs.shape))
        # print('pool vec shape: {}'.format(pooled_actions.shape))
        action_obs_embed = tf.concat([action_embed,tf.cast(embs,tf.float32),tf.cast(pooled_actions,tf.float32)],-1)
        # print('final vec shape: {}'.format(action_obs_embed.shape))
        # print(action_obs_embed.shape)
        action_logits = self.action_logits_model(action_obs_embed)
        # print('logits shape: {}'.format(action_logits.shape))
        # print(action_logits)
        #scaling modifications
        inf_mask = tf.maximum(tf.math.log(action_mask), tf.float32.min)
        logit_out = action_logits + tf.cast(inf_mask,tf.float32)
        ...
        if self.save_embeddings is not None:
            self.save(tf.squeeze(embedded_actions))
            # logz = tf.keras.activations.softmax(action_logits + tf.cast(inf_mask,tf.float32))[0]
            # for i in range(len(logz)):
            #     print('{}, logit: {}'.format(self.dgl_graph.ndata['_name'][i].numpy(),logit_out[0][i]))
